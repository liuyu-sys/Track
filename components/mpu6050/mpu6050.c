...
{
    i2c_master_write_read_device(i2c_num, MPU6050_SLAVE_ADDRESS, &reg_addr, 1, data_rd, size, MPU6050_TICKS_TO_WAIT);
}
...
static void i2c_mpu6050_write(i2c_port_t i2c_num, uint8_t reg_addr, uint8_t *data_wr, size_t size)
{
    ...
    uint8_t write_buf[2] = {reg_addr, *data_wr};
    i2c_master_write_to_device(i2c_num, MPU6050_SLAVE_ADDRESS, write_buf, sizeof(write_buf), MPU6050_TICKS_TO_WAIT);
}
